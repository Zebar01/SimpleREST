...
app.use('/api/query', function(req, res){
    ...
    let opt = {
        ...
        district:       req.query.district       || 1,		//Район Москвы для выборки
													/* ид районов
 ...
 //       page:           req.query.p             || '',
    ...
    let pathjs =__dirname+`/public/json/cian/`+god+'-'+mes+'-'+den;       //год-месяц-день
    mktemp.createDir(__dirname+`/public/json/cian/`+god+'-'+mes+'-'+den,function(err,pathjs){
      if (err) throw err;
 ...
 /*   if (opt.page='1'){            //заглушка для будущего перебора страниц
        opt.page ='&p=1';
    }*/
    ...
    request({
        ...
        if(response && response.statusCode && response.statusCode == 200) {
            ...
            try{
                ...
                fs.writeFileSync(pathjs+'/'+`${opt.district}`+'-'+`${opt.kolkom}`+'.json', JSON.stringify(obj)); /*сохранение json района по кол комнат*/
